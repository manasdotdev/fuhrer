import { type Middleware, type VirtualElement, arrow, autoPlacement, computePosition, flip, hide, inline, offset, shift, size } from '@floating-ui/dom';
...
export interface BubbleMenuPluginProps {
   ...
   */
  shouldShow?: ((props: { editor: Editor; element: HTMLElement; view: EditorView; state: EditorState; oldState?: EditorState; from: number; to: number }) => boolean) | null;
  ...
  // [modified] When true, freeze menu position on first show for a selection.
...
export class BubbleMenuView implements PluginView {
  ...
  // [modified] Cached selection rect used when lockPosition is enabled.
  private lockedRect: DOMRect | null = null;

  // [modified] Cached menu coordinates — prevents any late updatePosition call from moving the menu.
  private lockedMenuPosition: { left: number; top: number; strategy: string } | null = null;

  // [modified] Whether to freeze position after the menu first appears.
  ...
  private get virtualElement(): VirtualElement | undefined {
    // [modified] Return frozen rect while position is locked for the current selection.
    if (this.lockedRect) {
      const rect = this.lockedRect;
      return {
    ...
    if (selection instanceof CellSelection) {
      ...
      const clientRect =
        fromDOM === toDOM
          ...
          : combineDOMRects((fromDOM as HTMLElement).getBoundingClientRect(), (toDOM as HTMLElement).getBoundingClientRect());
  ...
  // [modified] Clear all frozen position state.
  private clearPositionLock() {
    this.lockedRect = null;
    this.lockedMenuPosition = null;
  }

  // [modified] Snapshot the live selection rect for lockPosition mode.
  private captureLockedRect() {
    const { selection } = this.editor.state;
    ...
    const domRect = posToDOMRect(this.view, selection.from, selection.to);
    this.lockedRect = new DOMRect(domRect.x, domRect.y, domRect.width, domRect.height);
  }
  ...
  constructor({
    ...
    if (this.getShouldShow()) {
      this.show();
      this.updatePosition();
      // [modified] Initial show bypasses updateHandler — freeze rect here too.
      if (this.lockPosition) {
        this.captureLockedRect();
      }
    }
  }
  ...
  resizeHandler = () => {
    ...
    this.resizeDebounceTimer = window.setTimeout(() => {
      // [modified] Skip reposition on scroll/resize when position is locked.
      if (!this.lockPosition) {
        this.updatePosition();
      }
    }, this.resizeDelay);
  ...
  focusHandler = () => {
    ...
    // [modified] Refresh visibility only — do not reposition on focus (e.g. after clicking a menu button).
    setTimeout(() => {
      if (this.getShouldShow()) {
  ...
  updatePosition() {
    ...
    // [modified] Restore frozen pixel position — blocks stray reposition calls while locked.
    if (this.lockPosition && this.lockedMenuPosition) {
      const { left, top, strategy } = this.lockedMenuPosition;
    ...
    computePosition(virtualElement, this.element, {
      ...
      // [modified] Freeze computed coordinates so later calls cannot move the menu.
      if (this.lockPosition) {
        this.lockedMenuPosition = { left: x, top: y, strategy };
      }
  ...
  update(view: EditorView, oldState?: EditorState) {
    ...
    // [modified] Missing oldState must not be treated as "everything changed" (focusHandler used to trigger this).
    const selectionChanged = oldState != null && !oldState.selection.eq(view.state.selection);
    const docChanged = oldState != null && !oldState.doc.eq(view.state.doc);
  ...
  updateHandler = (view: EditorView, selectionChanged: boolean, docChanged: boolean, oldState?: EditorState) => {
    ...
    // [modified] Reposition on first show, or pure selection moves — never on doc-only formatting (heading, bold, etc.).
    const wasVisible = this.isVisible;
    const shouldReposition = !wasVisible || (selectionChanged && !docChanged);

    if (selectionChanged && !docChanged && this.lockPosition) {
      // [modified] New selection — discard stale frozen state before recomputing position.
      this.clearPositionLock();
    }
    ...
    if (shouldReposition) {
      ...
      // [modified] Re-freeze selection rect after async position settles.
      if (this.lockPosition) {
        this.captureLockedRect();
      }
    }
  ...
  hide() {
    ...
    // [modified] Clear frozen state so the next show computes a fresh position.
    this.clearPositionLock();
  }
  ...
  transactionHandler = ({ transaction: tr }: { transaction: Transaction }) => {
    ...
    if (meta === 'updatePosition') {
      // [modified] Respect position lock unless explicitly forced while hidden.
      if (!this.lockPosition || !this.lockedMenuPosition) {
        this.updatePosition();
      }
  ...
  updateOptions(newProps: Partial<Omit<BubbleMenuPluginProps, 'editor' | 'element' | 'pluginKey'>>) {
    ...
    // [modified]
    if (newProps.lockPosition !== undefined) {
      this.lockPosition = newProps.lockPosition;
    }
